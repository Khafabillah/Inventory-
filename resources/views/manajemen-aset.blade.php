...
@section('content')
    ...
    {{-- Berisi judul halaman dan sub-judul deskriptif              --}}
    {{-- ========================================================== --}}
    ...
    {{-- ========================================================== --}}
    {{-- DATA DUMMY ASET                                            --}}
    {{-- Dideklarasikan di atas agar dapat diakses oleh kedua view  --}}
    {{-- ========================================================== --}}
    @php
        $dataAset = [
    ...
    {{-- Div ini hanya akan dirender pada perangkat mobile          --}}
    {{-- ========================================================== --}}
    <div class="block lg:hidden px-4 mt-4">

        {{-- 1. Navigasi Filter Cabang (Scroll Horizontal) --}}
        <div class="flex overflow-x-auto gap-2 pb-2 scrollbar-hide">
            @foreach (['Semua', 'WML', 'WLD', 'WLS', 'TOSS', 'WLP'] as $index => $cabang)
                <button
                    class="px-5 py-1.5 text-xs font-bold {{ $index === 0 ? 'bg-[#D5E7FD] text-[#006EC4]' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50' }} rounded-full shadow-sm transition-all active:scale-95">
                    {{ $cabang }}
                </button>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
            @foreach ($dataAset as $item)
                <div
                    class="min-h-[140px] flex flex-col justify-between rounded-lg bg-white p-3 shadow-sm border border-gray-100">
                    {{-- Bagian Atas: Nama, ID, Kategori, dan Kondisi --}}
                    <div>
                        <div class="text-sm font-bold text-black">{{ $item[2] }}</div>
                        <div class="text-xs text-gray-400 mt-0.5">{{ $item[0] }}</div>
                        <div class="text-xs text-gray-500 mt-0.5">{{ $item[1] }}</div>

                        {{-- Kondisi ditaruh di bawah Kategori --}}
                        <div class="mt-2">
                            <span
                                class="inline-flex items-center justify-center text-center rounded-full px-2 py-0.5 text-[10px] font-bold {{ $item[4] == 'Baik' ? 'bg-green-100 text-green-700' : ($item[4] == 'Kurang Baik' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700') }}">
                                {{ $item[4] }}
                            </span>
                        </div>
                    </div>

                    {{-- Bagian Bawah: Lokasi Ruangan --}}
        ...
        {{-- mt-8 untuk memastikan jarak yang cukup di bawah --}}
        <div class="flex flex-col items-center gap-3 pb-4 mt-8 mb-4">
            <div class="text-xs text-gray-600">Menampilkan 1 - 10 dari 868 aset</div>
            <div class="flex items-center gap-4">
                <button type="button" class="text-gray-600">&lt;</button>
                <div class="inline-flex divide-x border border-gray-200 rounded-lg overflow-hidden">
                    <button type="button" class="px-3 py-2 text-sm text-gray-700 bg-gray-100">1</button>
                    <button type="button" class="px-3 py-2 text-sm text-gray-700">2</button>
                    <button type="button" class="px-3 py-2 text-sm text-gray-700">3</button>
                </div>
                <button type="button" class="text-gray-600">&gt;</button>
            </div>
        </div>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden mt-6">
            ...
            <div class="flex items-center justify-between bg-[#9FC7E7] py-3 px-4 text-sm text-gray-900">
                ...
                <div class="inline-flex items-center gap-2">
                    ...
                    <span
                        class="inline-flex h-9 w-9 items-center justify-center rounded border border-black bg-white"></span>
                    <span
                        class="inline-flex h-9 w-9 items-center justify-center rounded border border-black bg-white"></span>
                    <span
                        class="inline-flex h-9 w-9 items-center justify-center rounded border border-black bg-white"></span>
