...
@section('content')
    ...
    <div class="flex items-center justify-start gap-2 mb-6 px-1 overflow-x-auto scrollbar-hide font-inter">
        {{-- Tab Akun (Aktif) --}}
        <a href="{{ route('master.akun') }}" class="px-6 py-2 text-xs font-bold bg-[#D5E7FD] text-[#006EC4] rounded-full shadow-sm transition-transform active:scale-95">
        ...
        {{-- Tab Ruangan --}}
        <button class="px-6 py-2 text-xs font-bold bg-white border border-gray-200 text-gray-500 hover:bg-gray-50 rounded-full transition-colors cursor-not-allowed">
            <div class="flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div class="min-h-[140px] flex flex-col justify-between rounded-lg bg-white p-3 shadow-sm border border-gray-100 relative">
                    <div>
                        <div class="flex justify-between items-start gap-1">
                            ...
                            <div class="relative">
                                <button type="button" onclick="toggleDropdown('dropdown-{{ $user->id }}')" class="p-1 text-gray-400 focus:outline-none">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <circle cx="12" cy="12" r="1" /><circle cx="12" cy="5" r="1" /><circle cx="12" cy="19" r="1" />
                                    </svg>
                                </button>
                                <div id="dropdown-{{ $user->id }}" class="hidden absolute right-0 top-6 mt-1 w-32 bg-white rounded-lg shadow-lg border border-gray-100 z-50 overflow-hidden">
                                    <div class="py-1">
                                        {{-- Tombol Edit Mobile --}}
                                        <button type="button" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-email="{{ $user->email }}" data-role="{{ $user->role }}" onclick="openEditUserModal(this); toggleDropdown('dropdown-{{ $user->id }}')" class="w-full text-left px-3 py-2 text-xs font-medium text-gray-600 hover:bg-blue-50 hover:text-blue-600 flex items-center gap-2">
                                        ...
                                        {{-- Tombol Hapus Mobile --}}
                                        <button type="button" data-id="{{ $user->id }}" data-name="{{ $user->name }}" onclick="openDeleteUserModal(this); toggleDropdown('dropdown-{{ $user->id }}')" class="w-full text-left px-3 py-2 text-xs font-medium text-gray-600 hover:bg-red-50 hover:text-red-600 flex items-center gap-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div class="hidden lg:block">
        <form method="GET" action="{{ route('master.akun') }}" class="flex items-center justify-between mb-6 gap-4 font-inter">
            ...
            <button type="button" onclick="toggleModal('modalTambahAkun')" class="cursor-pointer inline-flex items-center gap-2 text-sm font-bold text-[#006EC4] hover:text-blue-800 transition-all">
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm font-inter">
                        ...
                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50/50' }} hover:bg-blue-50/50 transition-colors">
                            ...
                            <td class="py-4">
                                <div class="flex justify-center gap-3 text-gray-500">
                                    {{-- Tombol Edit Desktop --}}
                                    <button type="button" title="Edit Profil" data-id="{{ $user->id }}" data-name="{{ $user->name }}" data-email="{{ $user->email }}" data-role="{{ $user->role }}" onclick="openEditUserModal(this)" class="cursor-pointer hover:text-blue-600 transition-colors focus:outline-none p-1 rounded hover:bg-blue-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z" /><path d="m15 5 4 4" />
                                        </svg>
                                    </button>
                                    {{-- Tombol Hapus Desktop --}}
                                    <button type="button" title="Hapus Akun" data-id="{{ $user->id }}" data-name="{{ $user->name }}" onclick="openDeleteUserModal(this)" class="cursor-pointer hover:text-red-600 transition-colors focus:outline-none p-1 rounded hover:bg-red-100">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                            <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6" /><path d="M3 6h18" /><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
                                        </svg>
                                    </button>
            ...
            {{-- Pagination Desktop: Sembunyikan jika <= 10, tampilkan teks ringkasan --}}
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center">
                <span class="text-xs text-gray-500">Menampilkan {{ $users->count() }} dari {{ $users->total() }} data</span>
                @if ($users->hasPages())
    ...
    <x-modal id="modalTambahAkun" title="Tambah Akun Baru">
        <form action="{{ route('master.akun.store') ?? '#' }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" required class="w-full border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500" placeholder="Masukkan nama lengkap..." />
            </div>
            <div>
                <label class="block text-sm text-gray-600 mb-1">Email</label>
                <input type="email" name="email" required class="w-full border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500" placeholder="nama@email.com" />
            </div>
            <div>
                <label class="block text-sm text-gray-600 mb-1">Role / Hak Akses</label>
                <select name="role" required class="w-full border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
            ...
            <div>
                <label class="block text-sm text-gray-600 mb-1">Password Sementara</label>
                <input type="password" name="password" required class="w-full border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500" placeholder="Minimal 8 karakter..." />
            </div>
            <div class="flex items-center justify-end gap-3 pt-4 border-t mt-4">
                <button type="button" onclick="toggleModal('modalTambahAkun')" class="px-4 py-2 bg-gray-100 text-gray-700 rounded text-sm hover:bg-gray-200 transition-colors">Batal</button>
                <button type="submit" class="px-4 py-2 bg-[#006EC4] text-white rounded text-sm hover:bg-blue-700 transition-colors">Simpan Akun</button>
            </div>
        </form>
    </x-modal>
    ...
    <x-modal id="modalEditAkun" title="Edit Profil Akun">
        <form id="formEditAkun" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-sm text-gray-600 mb-1">Nama Lengkap</label>
                <input type="text" name="name" id="edit_user_name" required class="w-full border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
            </div>
            <div>
                <label class="block text-sm text-gray-600 mb-1">Email</label>
                <input type="email" name="email" id="edit_user_email" required class="w-full border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
            </div>
            <div>
                <label class="block text-sm text-gray-600 mb-1">Role / Hak Akses</label>
                <select name="role" id="edit_user_role" required class="w-full border border-gray-200 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
                    <option value="Super Admin">Super Admin</option>
                    <option value="Store Manager">Store Manager</option>
                    <option value="Staff IT">Staff IT</option>
                    <option value="Staff Gudang">Staff Gudang</option>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t mt-4">
                <button type="button" onclick="toggleModal('modalEditAkun')" class="px-4 py-2 bg-gray-100 text-gray-700 rounded text-sm hover:bg-gray-200">Batal</button>
    ...
    <x-modal id="modalHapusAkun" title="Konfirmasi Hapus Akun">
        <form id="formHapusAkun" method="POST">
            ...
            <div class="py-4 text-center flex flex-col items-center">
                ...
                <p class="text-sm text-gray-500">Anda akan menghapus akun <br><strong id="delete_user_name" class="text-red-600"></strong></p>
            ...
            <div class="flex items-center justify-center gap-3 pt-4 border-t mt-2">
                <button type="button" onclick="toggleModal('modalHapusAkun')" class="px-6 py-2 bg-gray-100 text-gray-700 font-medium rounded text-sm hover:bg-gray-200 transition-colors">Batal</button>
