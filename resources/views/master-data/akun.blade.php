...
@section('content')
    ...
    <div class="font-inter flex flex-col mt-2 mb-6 px-1">
        ...
        <p class="text-[13px] font-light text-[#6B7280] ml-1">Kelola Parameter Ruangan, Kategori Aset, dan Hak Akses Pengguna
        </p>
    </div>
    ...
    <div class="hidden lg:block">
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm font-inter">
            ...
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center">
                <span class="text-xs text-gray-500">Menampilkan {{ $users->count() }} dari {{ $users->total() }}
                    data</span>
                @if ($users->hasPages())
    ...
    <x-modal id="modalEditAkun" title="Edit Profil Akun">
        <form id="formEditAkun" method="POST" class="space-y-4">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t mt-4">
                ...
                <button type="submit" class="px-4 py-2 bg-[#006EC4] text-white rounded text-sm hover:bg-blue-700">Simpan
                    Perubahan</button>
            </div>
        </form>
    </x-modal>
